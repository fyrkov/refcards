...
3. S3 **Transfer Acceleration** increases transfer speed by transferring a file to an AWS "edge location" and then forwarding it to a target bucket in a target region.
...
In this case a requester must be an another AWS account.
